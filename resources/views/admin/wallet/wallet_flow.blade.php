        ...
        <form class="form-search">
            <span>
                ...
                <select name="flow_type" id="type" class="nav-search-input">
                    ...
                    <option value="{{ $k }}" @if(request('flow_type') == "$k") selected @endif>{{ $type }}</option>
                        @endforeach
                </select>
    ...
    <table id="simple-table" class="table  table-bordered table-hover">
        ...
        @foreach($flows as $flow)
            <tr>
                ...
                <td class="center">{{ $flow->symbol == 1 ? $flow->flow_number : -1* $flow->flow_number }}</td>
                <td class="center">{{ $flow->title  }}</td>
                <td class="center">{{ $flow->fee }}</td>
                <td class="center">{{ $flow->wallet_type == 1 ? '场内' : '可提' }}</td>
