...
@section('title',  $token->token_id ? '编辑代币' : '创建代币' )
@section('content')
        ...
        <form class="form-horizontal" role="form" method="post" action="{{ route('token.update', $token->token_id) }}">
            ...
            @else
                <form class="form-horizontal" role="form" method="post" action="{{ route('token.store') }}">
                        ...
                        <div class="col-sm-9">
                            <select type="text" name="coin_id" placeholder="代币名称" class="col-xs-10 col-sm-5">
                                ...
                                @foreach($coins as $coin)
                                    <option value="{{ $coin->coin_id }}" {{ $token->coin_id == $coin->coin_id ? 'selected' : '' }}>{{ $coin->coin_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> 代币的总发行量 </label>
                        <div class="col-sm-9">
                            <input type="text" name="token_total_supply" id="form-field-1" placeholder="代币的总发行量" class="col-xs-10 col-sm-5" value="{{ $token->token_total_supply }}" />
                        </div>
                    </div>
