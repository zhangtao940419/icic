...
{{--        <form class="form-horizontal" role="form" method="post" action="{{ route('token.update', $token->token_id) }}">
            <input type="hidden" name="_method" value="PUT">--}}
                <form class="form-horizontal" role="form" method="post" action="{{ route('InvestmentRule.store') }}">
                    {{ csrf_field() }}
                        ...
                        <div class="col-sm-9">
                            <select type="text" name="invest_id" placeholder="理财类型" class="col-xs-10 col-sm-5">
                                ...
                                @foreach($investmentType as $investment)
                                    <option value="{{ $investment['invest_id'] }}">{{ $investment['invest_type_name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
{{--                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="invest_type_name" id="form-field-1" placeholder="理财套餐名称" class="col-xs-10 col-sm-5" value="" />
                        </div>
                    </div>--}}
                    <div class="form-group">
                        <label class="col-sm-3 control-label no-padding-right" for="form-field-1">套餐时长</label>
                        <div class="col-sm-9">
                            <input type="text" name="invest_time" id="form-field-1" placeholder="套餐时长(以天为单位)" class="col-xs-10 col-sm-5" value="" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-3 control-label no-padding-right" for="form-field-1">利率</label>
                        <div class="col-sm-9">
                            <input type="text" name="rate_of_return_set" id="form-field-1" placeholder="利率(0~100)" class="col-xs-10 col-sm-5" value="" />
                        </div>
                    </div>
{{--
                    <div class="space-4"></div>
